...
export function StorybookPanel() {
  ...
  const [nPages, setNPages] = useState<number>(6);
  ...
  // Each page: ~7s Flux image + ~5-6 min Wan animation at 832×480. Plus stitching.
    ...
    <div className="flex flex-col gap-6">
      <div className="bg-gradient-to-br from-brand/10 to-orange-300/10 border border-brand/20 rounded-2xl p-4">
        ...
        <p className="text-xs text-fg-muted">
          Tell us a story. We'll illustrate {nPages} pages and animate them into a video.
        </p>
      </div>
      ...
      <Section title="How many pages?">
        <div className="grid grid-cols-3 gap-2">
          {PAGE_COUNTS.map((n) => {
            const active = n === nPages;
            return (
              <button
                key={n}
                onClick={() => setNPages(n)}
